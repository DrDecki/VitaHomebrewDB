...
if not rows:
    ...
    out.append('That took a lot of people. LiEnby kept an old cbpsdb mirror alive that still had' + chr(10))
    out.append('files nobody else could find. ChassyFlaps went through the list by hand and sent' + chr(10))
    out.append('back a page of sources. devnoname120 turned up PSVitaStuff, the last one missing.' + chr(10))
    out.append('Cimmerian-Iter pushed for verifying anything handed over, which is why every file' + chr(10))
    out.append('here was matched against the size VitaDB recorded. GameBrew and Brewology had kept' + chr(10))
    out.append('copies for years. Several authors just sent their own builds when asked.' + chr(10) + chr(10))
    out.append('Thanks.' + chr(10) + chr(10))
    out.append('If something breaks again, or you spot an entry pointing at the wrong file,' + chr(10))
    out.append('open an issue. This page fills up again when it needs to.' + chr(10))
